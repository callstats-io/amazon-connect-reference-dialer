...
const Quality = ({ qualityValue = 0, requestConnectivityCheck }) => (
  <div className={`col-6 p-0 ${styles.qualityBox}`}>
    <div className={`row no-gutters`}>
      <span className={`col-12 ${styles.bandwidthQuality}`}> Bandwidth quality: <span
    ...
    </div>
    {
      qualityValue >= 0 && qualityValue < 3 && <div className={`row`}>
        <a className={`btn col-12 align-self-left text-center mt-0 pt-0 ${styles.reportACall}`}
