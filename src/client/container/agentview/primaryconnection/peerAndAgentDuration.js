...
const PeerAndAgentDuration = ({ currentState = undefined, remoteStream = undefined }) => (
  <div className={`col-12 ${styles.peerDurationHeight}`}>
    <div className={`row`}>
      <div className={`col-6`}>
      ...
      <div className={`col-6 align-self-center pr-0 mr-0`}>
        {
          showPhoneNumber(currentState) &&
					<span className={`m-0 ${styles.peerAndAgentDurationText}`}>{sessionManager.getPrimaryConnectionPhone()}</span>
        }
      </div>
      <div className={`col-2 pl-0 ml-0 text-center`}>
