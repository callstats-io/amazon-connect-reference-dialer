...
(function(global) {
  var CallstatsAmazonShim = function(callstats) {
    ...
    function initPCShim() {
      ...
      if (window.RTCPeerConnection.prototype.csiogetStats) {
        window.RTCPeerConnection.prototype.getStats =
          window.RTCPeerConnection.prototype.csiogetStats;
      }
    }

    function subscribeToAmazonContactEvents(contact) {
      confId = contact.getContactId();
      CallstatsAmazonShim.remoteId =
      ...
      if (!confId) {
        confId =
          CallstatsAmazonShim.localUserID + ":" + CallstatsAmazonShim.remoteId;
      }
      ...
      contact.onEnded(function() {
        if (!isCallDetailsSent) {
          CallstatsAmazonShim.callstats.sendCallDetails(
            csioPc,
            confId,
            callDetails
          );
      ...
      contact.onRefresh(currentContact => {
        ...
        if (callInHold !== isOnHold) {
          ...
          callInHold = isOnHold;
          if (isOnHold) {
            CallstatsAmazonShim.callstats.sendFabricEvent(
              csioPc,
              CallstatsAmazonShim.callstats.fabricEvent.fabricHold,
              confId
            );
          } else {
            CallstatsAmazonShim.callstats.sendFabricEvent(
              csioPc,
              CallstatsAmazonShim.callstats.fabricEvent.fabricResume,
              confId
            );
          }
        }
    ...
    function getCallLogsAndSend() {
      let connectLogs = connect.getLog();
      console.log("Connect Logs: ", connectLogs);
      try {
        let count = 0;
        let logMessage = "";

        for (let i = lastLogLine; i < connectLogs._logs.length; i++) {
          logMessage += `[${connectLogs._logs[i].component} ${
            connectLogs._logs[i].level
          }] ${connectLogs._logs[i].text}\n`;

          count++;
          lastLogLine = i;
        }

        let logSize = byteCount(logMessage);
        console.log("Log Size", logSize);
        window.CallstatsAmazonShim.sendLogs(logMessage);

        if (count == 1) {
          console.log("Sent 1 log entry");
        } else {
          console.log("Sent " + count + " log entries");
        }
      } catch (e) {
        console.log(e);
      }
    ...
    function handleOnMuteToggle(obj) {
      ...
      if (obj) {
        if (obj.muted) {
          CallstatsAmazonShim.callstats.sendFabricEvent(
            csioPc,
            CallstatsAmazonShim.callstats.fabricEvent.audioMute,
            confId
          );
        } else {
          CallstatsAmazonShim.callstats.sendFabricEvent(
            csioPc,
            CallstatsAmazonShim.callstats.fabricEvent.audioUnmute,
            confId
          );
        }
      }
    ...
    function handleErrors(error) {
      ...
      if (!conferenceId) {
        conferenceId =
          CallstatsAmazonShim.localUserID +
          ":" +
          (CallstatsAmazonShim.remoteId || CallstatsAmazonShim.localUserID);
      }
      if (error.errorType === SoftphoneErrorTypes.MICROPHONE_NOT_SHARED) {
        CallstatsAmazonShim.callstats.reportError(
          null,
          conferenceId,
          CallstatsAmazonShim.callstats.webRTCFunctions.getUserMedia,
          error
        );
      } else if (
        error.errorType === SoftphoneErrorTypes.SIGNALLING_CONNECTION_FAILURE
      ) {
        CallstatsAmazonShim.callstats.reportError(
          null,
          conferenceId,
          CallstatsAmazonShim.callstats.webRTCFunctions.signalingError,
          error
        );
      } else if (
        error.errorType === SoftphoneErrorTypes.SIGNALLING_HANDSHAKE_FAILURE
      ) {
        CallstatsAmazonShim.callstats.reportError(
          csioPc,
          conferenceId,
          CallstatsAmazonShim.callstats.webRTCFunctions.setLocalDescription,
          error
        );
        CallstatsAmazonShim.callstats.sendCallDetails(
          csioPc,
        ...
        error.errorType === SoftphoneErrorTypes.ICE_COLLECTION_TIMEOUT
      ) {
        CallstatsAmazonShim.callstats.reportError(
          csioPc,
          conferenceId,
          CallstatsAmazonShim.callstats.webRTCFunctions.iceConnectionFailure,
          error
        );
        CallstatsAmazonShim.callstats.sendCallDetails(
          csioPc,
        ...
        switch (error.endPointUrl) {
          case RTCErrorTypes.SET_REMOTE_DESCRIPTION_FAILURE:
            CallstatsAmazonShim.callstats.reportError(
              csioPc,
              conferenceId,
              CallstatsAmazonShim.callstats.webRTCFunctions
    ...
    function handleSessionCreated(pc) {
      ...
      try {
        CallstatsAmazonShim.callstats.addNewFabric(
          csioPc,
          CallstatsAmazonShim.remoteId,
          CallstatsAmazonShim.callstats.fabricUsage.multiplex,
          confId,
          fabricAttributes
        );
      } catch (error) {
    ...
    CallstatsAmazonShim.prototype.initialize = function initialize(
      connect,
      appID,
      appSecret,
      localUserID,
      ...
      CallstatsAmazonShim.callstats.initialize(
        appID,
        appSecret,
        localUserID,
        csInitCallback,
    ...
    CallstatsAmazonShim.prototype.sendUserFeedback = function sendUserFeedback(
      feedback,
      callback
    ) {
      if (!confId) {
      ...
      CallstatsAmazonShim.callstats.sendUserFeedback(
        confId,
        feedback,
        callback
      );
    ...
    CallstatsAmazonShim.prototype.sendFabricEvent = function sendFabricEvent(
      fabricEvent,
      eventData
    ) {
      if (!csioPc || !confId) {
    ...
    CallstatsAmazonShim.prototype.sendLogs = function sendLogs(domError) {
      ...
      CallstatsAmazonShim.callstats.reportError(
        csioPc,
        confId,
        CallstatsAmazonShim.callstats.webRTCFunctions.applicationError,
        domError
      );
    };

    CallstatsAmazonShim.prototype.makePrecallTest = function makePrecallTest(
      precallTestResultsCallback
    ) {
      ...
      CallstatsAmazonShim.callstats.on(
        "preCallTestResults",
        precallTestResultsCallback
      );
